...
def check_intrinsics(data_root, train, val):
    ...
    for i,item in enumerate(info['data_list']):
        for img in keys:
            K = item['images'][img]['cam2img']
            fx = K[0][0]
            fy = K[1][1]
            focal[img]['fx'].append(fx)
            focal[img]['fy'].append(fy)
    for img in keys:
        fx = torch.tensor(focal[img]['fx']).mean()
        fy = torch.tensor(focal[img]['fy']).mean()
        print('{} has fx: {}, fy: {}'.format(img,fx,fy))
