...
import torch
import mmengine
#BUG: still can not align with official nuscenes evaluation metric. DO NOT USE IN PAPER!
@DATASETS.register_module()
class NuScenesEval_native:
    def __init__(self, 
                 save_loc = 'results/temp_nusc/',
                 classes=['car', 'pedestrian', 'cyclist'],  # then car for 0, ped for 1, cyc for 2
    ...
    def lidar_instances_to_np(self, instances, is_pred=False):
        bboxes = np.array(instances['bboxes_3d'].tensor[:,:7])
        bboxes[:,2] += bboxes[:,5]/2
        labels = np.array(instances['labels_3d'],dtype=int)
        scores = np.array(instances.get('scores_3d'))
        if is_pred:
            return {'bboxes_3d': bboxes,'labels_3d': labels,'scores_3d': scores}
        else:
            return {'bboxes_3d': bboxes,'labels_3d': labels}
    ...
    def evaluate(self, results):
        ...
        for i, result in enumerate(results):
            ...
            predictions = self.lidar_instances_to_np(result['pred_instances_3d'], is_pred=True)
            ground_truth = self.lidar_instances_to_np(result['gt_instances'])
        # Filter range
            if self.max_range > 0:
    ...
    def eval_pair(self, pred_label, gt_label):
        ...
        ## Threshold score
        mask = (pred_label['scores_3d'] > self.score_threshold)
        pred_label = self.filter_boxes(pred_label, mask)

        for cls_idx, single_class in enumerate(self.classes):
            # get all pred labels, order by score
            pred_mask = (pred_label['labels_3d'] == cls_idx)
            class_pred_label = pred_label['bboxes_3d'][pred_mask]
            score = pred_label['scores_3d'][pred_mask]
            class_pred_label = np.hstack((class_pred_label, score.reshape(-1,1)))
            class_pred_label = class_pred_label[(-score).argsort()].astype(np.float) # sort decreasing

            # add gt label length to total_N_pos
            gt_mask = (gt_label['labels_3d'] == cls_idx)
            class_gt_label =  gt_label['bboxes_3d'][gt_mask]
    ...
    def filter_boxes(self, labels, mask):
        for key in labels:
    ...
    def filter_by_range(self, pred_label, gt_label, range=0):
        ...
        pred_label = self.filter_boxes(pred_label, pred_dist)
        gt_label = self.filter_boxes(gt_label, gt_dist)
