...
for i,z in enumerate(Zs):
    for j,x in enumerate(Xs):
        work_dir = './work_dirs_3.00mv_ablate/ANK_KRt+egoAll_eval_on_LKK360_z{}_x{}'.format(z,x)
        src_cfg = 'projects/configs/ablation_3.00mv_test/KRt+egoAll.py'
        ckpt = 'work_dirs_ablate/KRt+egoAll/epoch_24.pth'
        NLmeta = dict(
...
for k,name in zip(range(2,8),['A','K','K360','L','N','W']):
    save_hist(waymo_brief[k], filename='egoXZ_KRt_mv_'+name)
breakpoint()
