...
base_dist = 51.2
base_fxfy = 2068.5
ego_aug_train = [dict(type='EgoTranslate', Tr_range = [-2,-2,-2,2,2,2], eval=False)]
ego_aug_eval = [dict(type='EgoTranslate', Tr_range = [-2,-2,-2,2,2,2])]
work_dir = './work_dirs_RtK/1.00argnuway_RtK_aware_Ksync_egoaug2z'
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        ...
                        dict(type='Detr3DCrossAtten',
                             ...
                             num_points=1,
                             embed_dims=256,
                             feature_sampler=dict(type='GeoAwareFeatSampler',
                                                  base_fxfy = base_fxfy,
                                                  base_dist = base_dist))
                    ],
...
argo2_train_pipeline = argo2_pipeline_default + ego_aug_train + [
...
argo2_test_pipeline = [dict(type='evalann2ann')] + argo2_pipeline_default + ego_aug_eval + [
    dict(type='ObjectRangeFilter', point_cloud_range=point_cloud_range),
...
nusc_train_pipeline = nusc_pipeline_default + ego_aug_train+ [
    dict(type='ObjectRangeFilter', point_cloud_range=point_cloud_range),
...
nusc_test_pipeline = [dict(type='evalann2ann')] + nusc_pipeline_default + ego_aug_eval + [
    dict(type='ObjectRangeFilter', point_cloud_range=point_cloud_range),
...
waymo_train_pipeline = waymo_pipeline_default + ego_aug_train + [
    dict(type='ObjectRangeFilter', point_cloud_range=point_cloud_range),
    dict(type='MultiViewWrapper', transforms=[dict(type='PhotoMetricDistortion3D')] + waymo_test_transforms),
    dict(type='Pack3DDetInputsExtra', keys=['img', 'gt_bboxes_3d', 'gt_labels_3d'])
]
waymo_test_pipeline = [dict(type='evalann2ann')] + waymo_pipeline_default + ego_aug_eval + [
    dict(type='ObjectRangeFilter', point_cloud_range=point_cloud_range),
