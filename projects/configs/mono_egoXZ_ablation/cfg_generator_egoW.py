...
waymo_brief = np.zeros((Zs.shape[0],Xs.shape[0]))
waymo_detail = np.array(waymo_brief,dtype=str)
for i,z in enumerate(Zs):
    for j,x in enumerate(Xs):
        work_dir = './work_dirs_mono_egoXZ_ablate/W_fx2070_eval_on_W_z{}_x{}'.format(z,x)
        src_cfg = 'projects/configs/mono_egoXZ_ablation/W_fx2070_eval_W.py'
        ...
        with open(work_dir+'/brief_metric.txt','r') as result_file:
            ...
            items = lines[1].strip('\n').split(' ')
            waymo_brief[i,j] = items[1][:-1]
            waymo_detail[i,j] = items[2]
