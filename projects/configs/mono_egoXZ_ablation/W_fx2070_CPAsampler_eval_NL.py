...
input_modality = dict(use_lidar=True, # True if debug_vis
                      use_camera=True)
argo_egoZ = dict(type='EgoTranslate', trans = [0,0,-0])
...
detr3d_feature_sampler = CPA_sampler
default_crossattn = dict(type='Detr3DCrossAtten',num_cams = 1)
...
joint_train = dict(
        ...
        datasets=[nusc_train, lyft_train])
joint_val = dict(
        type='CustomConcatDataset',
        datasets=[nusc_val, lyft_val])
