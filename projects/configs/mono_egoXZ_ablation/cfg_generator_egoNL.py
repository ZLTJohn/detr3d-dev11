...
for i,z in enumerate(Zs):
    for j,x in enumerate(Xs):
        ...
        with open('projects/configs/mono_egoXZ_ablation/W_fx2070_eval_NL.py','r') as f:
            s = f.readlines()
            with open('debug/temp.py','a') as f1:
                f1.writelines(s)
        # run
        # child = subprocess.Popen(['tools/dist_test.sh','debug/temp.py','work_dirs_extended/1.00W_front_r50_fullres/epoch_24.pth' ,'8'])
        # child.wait()
        # get results
