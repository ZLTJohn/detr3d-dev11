...
for i,z in enumerate(Zs):
    for j,x in enumerate(Xs):
        work_dir = './work_dirs_mono_egoXZ_ablate/W_fx2070_CPAsampler_z{}_x{}'.format(z,x)
        NLmeta = dict(
        ...
        cfg = Config(NLmeta)
        Config.dump(cfg,'debug/temp3.py')
        with open('projects/configs/mono_egoXZ_ablation/W_fx2070_CPAsampler_eval_NL.py','r') as f:
            s = f.readlines()
            with open('debug/temp3.py','a') as f1:
                f1.writelines(s)
        # run
        # child = subprocess.Popen(['tools/dist_test.sh','debug/temp3.py','work_dirs_mono_ablate/1.00W_fx2070_CPAsampler_front/epoch_24.pth' ,'8'])
        # child.wait()
        # get results
