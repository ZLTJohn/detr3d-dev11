...
Zs = np.array(list(range(-5,11,5))) / 10.0
Xs = np.array(list(range(0,1,5))) / 10.0
...
for i,z in enumerate(Zs):
    for j,x in enumerate(Xs):
            # work_dir = './work_dirs_mono_egoXZ_ablate/W_fx2070_z{}_x{}'.format(z,x)
            work_dir = './work_dirs_mono_egoXZ_ablate/debug'
            NLmeta = dict(
            ...
            cfg = Config(NLmeta)
            Config.dump(cfg,'debug/temp.py')
            with open('projects/configs/mono_egoXZ_ablation/debug.py','r') as f:
                s = f.readlines()
                with open('debug/temp.py','a') as f1:
                    f1.writelines(s)
            # run
            child = subprocess.Popen(['tools/dist_test.sh','debug/temp.py','work_dirs_extended/1.00W_front_r50_fullres/epoch_24.pth' ,'1'])
            child.wait()
            # get results
