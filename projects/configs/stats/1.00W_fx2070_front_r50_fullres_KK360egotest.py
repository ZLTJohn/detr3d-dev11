...
kitti_egoZ = [dict(type='ego_transform', trans = 'Tr_imu_to_velo'),dict(type='EgoTranslate', trans = [0.05,-0.32,-0.80])]
nusc_egoZ = dict(type='ego_transform')
K360_egoZ = dict(type='ego_transform')
lyft_egoZ = dict(type='ego_transform')
work_dir = './work_dirs_mono_egotest/debug'
# resume = True
argo2_name_map = {
    'REGULAR_VEHICLE': 'Car',
    'LARGE_VEHICLE': 'Car',
...
joint_train = dict(
        ...
        datasets=[kitti_train,waymo_train])
joint_val = dict(
        type='CustomConcatDataset',
        datasets=[kitti_val,kitti_train])
