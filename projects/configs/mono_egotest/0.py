...
input_modality = dict(use_lidar=True, # True if debug_vis
                      use_camera=True)
argo_egoZ = dict(type='EgoTranslate', trans = [0,0,0.3217])
kitti_egoZ = dict(type='EgoTranslate', trans = [0,0,-0.9703])
K360_egoZ = dict(type='EgoTranslate', trans = [0,0,-0.8209])
lyft_egoZ = dict(type='EgoTranslate', trans = [0,0,-1.8597])
waymo_egoZ = dict(type='EgoTranslate', trans = [0,0,0.6272])
work_dir = './work_dirs_mono_egotest/1.00N_fx2070_egoZSync_through_labelZ'
# resume = True
argo2_name_map = {
    'REGULAR_VEHICLE': 'Car',
    'LARGE_VEHICLE': 'Car',
