...
input_modality = dict(use_lidar=True, # True if debug_vis
                      use_camera=True)
argo_egoZ = [dict(type='EgoTranslate', trans = [0,0,-0.3488])]
kitti_egoZ = [dict(type='EgoTranslate', trans = [0,0,-1.73])]
K360_egoZ = [dict(type='EgoTranslate', trans = [0,0,-1.73])]
nusc_egoZ = [dict(type='ego_transform'), dict(type='EgoTranslate', trans = [0,0,0])]
lyft_egoZ = [dict(type='ego_transform'), dict(type='EgoTranslate', trans = [0,0,0])]
work_dir = './work_dirs_extended/6.00ANWLKK360_front_r50_fx2070_groundZ'
# resume = True
argo2_name_map = {
    'REGULAR_VEHICLE': 'Car',
    'LARGE_VEHICLE': 'Car',
